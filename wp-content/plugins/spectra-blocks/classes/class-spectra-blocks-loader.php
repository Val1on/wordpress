...
class Spectra_Blocks_Loader {
	...
	private static function setup_hooks() {
		...
		// Register AI modules with zip-ai so ai_assistant and ai_design_copilot
		// can be enabled/disabled and the editor sidebar is aware of them.
		add_filter( 'zip_ai_modules', array( __CLASS__, 'register_zip_ai_modules' ), 20 );
	}
	...
	/**
	 * Register ai_assistant and ai_design_copilot modules with the zip-ai library.
	 *
	 * Without this, Module::is_enabled() always returns false and the AI sidebar
	 * button never appears in the block editor.
	 *
	 * @since x.x.x
	 * @param array $modules Existing registered modules.
	 * @return array
	 */
	public static function register_zip_ai_modules( $modules ) {
		if ( ! is_array( $modules ) ) {
			$modules = array();
		}
		...
		foreach ( array( 'ai_assistant', 'ai_design_copilot' ) as $module ) {
			\ZipAI\Classes\Module::force_enabled( $modules, $module );
		}
	...
	private static function load_versioned_lib( $version_file, $lib_key, $entry_file, $hook = 'init', $priority = 10 ) {
		...
		if ( null === $$var_version ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
			$$var_version = '0'; // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
		}

		if ( version_compare( $version, $$var_version, '>=' ) ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
			$$var_version = $version; // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
			$$var_path    = $path; // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
		}
