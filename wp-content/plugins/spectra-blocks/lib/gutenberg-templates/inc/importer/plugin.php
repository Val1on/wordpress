...
class Plugin {
	...
	public function api_request() {
		...
		if ( is_wp_error( $response ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'Something went wrong', 'spectra-blocks' ),
	...
	public function import_wpforms( $wpforms_url = '' ) {
		...
		if ( ! empty( $wpforms_url ) && function_exists( 'wpforms_encode' ) ) {
			...
			wp_send_json_error( __( 'Something went wrong', 'spectra-blocks' ) );
		}
	...
	public function import_sureforms() {
		...
		if ( empty( $sureforms_url ) ) {
			wp_send_json_error( __( 'Empty SureForms URL', 'spectra-blocks' ) );
		}
	...
	public function activate_plugin() {
		...
		if ( empty( $plugin_init ) ) {
			wp_send_json_error( __( 'Plugin slug is missing.', 'spectra-blocks' ) );
		}
	...
	public function template_importer() {
		...
		// Early return.
		if ( '' == $api_uri ) {
			wp_send_json_error( __( 'Something wrong', 'spectra-blocks' ) );
		}
		...
		wp_localize_script(
			...
			apply_filters(
				...
				array(
					...
					'button_text'             => esc_html__( 'Design Library', 'spectra-blocks' ),
	...
	public function get_page_palette_colors() {
		...
		$color_palettes = array(
			'style-1' =>
				array(
					'slug'          => 'style-1',
					'title'         => __( 'Light', 'spectra-blocks' ),
					'default_color' => $default_palette_color[ $reorganize ? 5 : 4 ],
					'colors'        => $palette_one,
				),
			'style-2' => array(
				'slug'          => 'style-2',
				'title'         => __( 'Dark', 'spectra-blocks' ),
	...
	public function get_block_palette_colors() {
		...
		$color_palettes = array(
			...
			'style-3' => array(
				'slug'          => 'style-3',
				'title'         => __( 'Highlight', 'spectra-blocks' ),
	...
	public function get_version_toggle_labels() {
		...
		if ( $spectra_blocks_active && $uagb_active ) {
			return array(
				'v2' => __( 'Spectra Legacy', 'spectra-blocks' ),
				'v3' => __( 'Spectra Blocks', 'spectra-blocks' ),
			);
		}

		return array(
			'v2' => __( 'v2', 'spectra-blocks' ),
			'v3' => __( 'v3', 'spectra-blocks' ),
		);
	}
